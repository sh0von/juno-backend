...
router.post('/register', [
    ...
    try {
        ...
        console.error(error);
        return res.status(500).send("Internal Server11 Error");
    }
});
...
router.post('/login', [
    ...
    try {
        ...
        console.error(error);
        return res.status(500).send("Internal Server22222 Error");
    }
});
...
router.post('/forgotpassword', [
    ...
    transporter.sendMail(options, (err, info) => {
        if (err) {
            return res.status(400).json({ error: "Internal33 Server Error!" });
            console.log(error);
        }
